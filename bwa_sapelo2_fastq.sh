...
#PBS -m abe                                                              
#PBS -o /scratch/rx32940/bwa_fastq_results                       
#PBS -e /scratch/rx32940/bwa_fastq_results                        
#PBS -j oe     
...
# cohort variant calling
        ...
        bcftools mpileup -Ou -f /scratch/rx32940/lepto_wgs_seq/Lai_56601.fasta -b $o_path/bam_list.list > $o_path/multisample.bcf
        #call snps
        bcftools call -mv -Ob $o_path/multisample.bcf > $o_path/multisample_final.bcf
        # filter those with quality score less than 20
        bcftools view -i '%QUAL>=150' $o_path/multisample_final.bcf > $o_path/multisample_final.vcf
